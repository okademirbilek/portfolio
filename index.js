...
const waterAmbientOcclusion = new URL(
  "./img/Water_002_OCC.jpg",
  import.meta.url
);
...
camera.position.set(36.749, 30.929, 84.7292);
...
ptag.id = "about--p";
ptag.textContent = `
        A  Front-End Developer trying to learn and 
...
myimg.id = "myimg";
myimg.src =
  "https://github.com/okademirbilek/dontdismantleit/blob/main/images/okan.jpg?raw=true";
...
function mainProject() {
  ...
  data.data.projects.map((item) => {
    return (div3.innerHTML += `<div key=${item.id} class="map-item">
                                <h3 class="projects--name">${item.name}</h3>
                                <div class="project--img--div">
                                    <img draggable="false"  id=${item.id} class="project--img" src=${item.url}" ><img>
                                </div>
                            </div>
            `);
  });
  ...
  for (const i of data.data.projects) {
    if (event.target.id == i.id) {
      ...
      div4.innerHTML = `<div class="project--div2">
                    ...
                    <img class="project--img2" draggable="false" src=${i.url}"><img>
...
const sphereMaterial = new THREE.MeshStandardMaterial({
  ...
  displacementMap: textureLoader.load(waterHeightMap.href),
  displacementScale: 0.01,
  roughnessMap: textureLoader.load(waterRoughness.href),
  roughness: 0,
    ...
    window.addEventListener("wheel", function () {
      ...
      switch (position) {
        case 0:
          ...
          break;
        case 1:
          moveCamera(36.749, 30.929, 84.7292);
        ...
        case 4:
          moveCamera(9.452, 23.75, 70.4134);
        ...
        case 6:
          moveCamera(-48.278, 75.81835, 1.0000831);
          rotateCamera(-0.181212, 0.190572, 0.034692);
          position = 7;
          break;
...
function animate(time) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // set new position
    sphereGeometry.attributes.position.setX(
      i,
      position_clone[ix] + normals_clone[ix] * (xsin + ycos)
    );
    sphereGeometry.attributes.position.setY(
      i,
      position_clone[iy] + normals_clone[iy] * (xsin + ycos)
    );
    sphereGeometry.attributes.position.setZ(
      i,
